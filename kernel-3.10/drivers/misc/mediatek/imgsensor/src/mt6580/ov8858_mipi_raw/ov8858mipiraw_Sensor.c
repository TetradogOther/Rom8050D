 ...
 *
 *	PengtaoFan
 *	20150715:LOG_INF ?邦D?log info 車?㏒?pr_debug?㏒for車D?赤?∫﹞∩角?2??邦?a?迆
 *	20150716:?邦D?213?∩辰車?log?D?a那∫米?2?那y
 *------------------------------------------------------------------------------
...
void OV8858GetModuleID(void);



/****************************Modify Following Strings for Debug****************************/
#define PFX "OV8858R2A4LANE"
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.cap1 = {							//capture for PIP 24fps relative information, capture1 mode must use same framelength, linelength with Capture mode for shutter calculate
		.pclk = 144000000,
		.linelength  = 2425,
		.framelength = 3960,//MODIFY FOR 15FPS FOR MT6735 //2556
		.startx = 0,
		.starty = 0,
		.grabwindow_width  = 3264,
		.grabwindow_height = 2448,
...
//  ;;
	...
	//[PRFIX :PR735317] chengming.xiang 2015-10-28 modify begin :some times sensor don't output data cause camera black screen
	//as ov8858 power up sequence timing constraints ,add 10ms delay after soft reset 
	mDELAY(10);
	//[PRFIX :PR735317] chengming.xiang 2015-10-28 modify end  
	...
	write_cmos_sensor(0x3768, 0xc2);//cc
	...
	write_cmos_sensor(0x4006, 0x04);//0x08 
	write_cmos_sensor(0x4007, 0x04);//0x08 lxd 0603
	write_cmos_sensor(0x4002, 0x27);
	write_cmos_sensor(0x4009, 0x81); //0x83
	...
	write_cmos_sensor(0x4020, 0x00);
	write_cmos_sensor(0x4021, 0x04);
	write_cmos_sensor(0x4022, 0x06);//0x04
	write_cmos_sensor(0x4023, 0x00);//0xb9
	write_cmos_sensor(0x4024, 0x0f);//0x05
	write_cmos_sensor(0x4025, 0x2a);
	write_cmos_sensor(0x4026, 0x0f);//0x05
	write_cmos_sensor(0x4027, 0x2b);
	write_cmos_sensor(0x4028, 0x00);
	write_cmos_sensor(0x4029, 0x02);
	write_cmos_sensor(0x402a, 0x04);
	write_cmos_sensor(0x402b, 0x04);
	write_cmos_sensor(0x402c, 0x00); //0x02
	write_cmos_sensor(0x402d, 0x02);
	write_cmos_sensor(0x402e, 0x04);//0x08
	write_cmos_sensor(0x402f, 0x04); //0x02
	...
	write_cmos_sensor(0x4837, 0x1a); //0x16
	...
	write_cmos_sensor(0x5784, 0x00);//0x01
...
//   100 99 1632 1224
	...
	write_cmos_sensor(0x37b4, 0x42);     //
	write_cmos_sensor(0x37b5, 0x31);     // ;;; 
	write_cmos_sensor(0x3768, 0xc2);     //0x22
	write_cmos_sensor(0x3769, 0x44);     //
	write_cmos_sensor(0x376a, 0x44);     //
	write_cmos_sensor(0x3808, 0x06);     //
	write_cmos_sensor(0x3809, 0x60);     //
...
//  100 99 3264 2448);    //
	...
	write_cmos_sensor(0x380f, 0x78);  //   ;aa
	...
	write_cmos_sensor(0x4020, 0x00);  //  
	write_cmos_sensor(0x4021, 0x04);  //  
	write_cmos_sensor(0x4022, 0x0c);  //  0x0b
	write_cmos_sensor(0x4023, 0x60);  //  0xc3
	write_cmos_sensor(0x4024, 0x0f);  //  0x0c
	write_cmos_sensor(0x4025, 0x36);  //  
	write_cmos_sensor(0x4026, 0x0f);  //  
	write_cmos_sensor(0x4027, 0x37);  //  
	write_cmos_sensor(0x402a, 0x04);  //  
	write_cmos_sensor(0x402b, 0x08);  //  
	write_cmos_sensor(0x402e, 0x04);  //  0x0c
	write_cmos_sensor(0x402f, 0x08);  //  0x02
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		printk("sensor_id_not_equal\n");
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}else
		{
			printk("zgxadd %s module_id=%d \n", __func__, module_id);
			if (module_id != 0x01) //0x01
				{
				printk("zgx1111 \n");
				*sensor_id = 0xFFFFFFFF;
				return ERROR_SENSOR_CONNECT_FAIL;
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		printk("ov8858 group1 \n");
		addr = 0x7011; // base address of info group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		
		printk("ov8858 group2 \n");
		//[PRFIX  PR967601 begin :module id should be 0x7018 ,not 0x7019 chengming.xiang@tcl.com 2015-11-23]
		addr = 0x7018; // base address of info group 2
		//[PRFIX  PR967601 end :module id should be 0x7018 ,not 0x7019 chengming.xiang@tcl.com 2015-11-23]
	}
	if(addr != 0) {
		printk("optoptopt \n");
		...
		temp = read_cmos_sensor(addr + 17);//AWB_Light_LSB1
		printk("zgx_temp = %d ,addr11 = 0x%d \n",temp,addr);
		(*otp_ptr).rg_ratio = (read_cmos_sensor(addr + 15)<<2) + ((temp>>6) & 0x03);//AWB_RG_MSB1
		printk("zgx_rg_ratio = %d ,addr222 = 0x%d \n",(*otp_ptr).rg_ratio,addr);
		(*otp_ptr).bg_ratio = (read_cmos_sensor(addr + 16)<<2) + ((temp>>4) & 0x03);//AWB_BG_MSB1
		printk("zgx_bg_ratio = %d \n",(*otp_ptr).bg_ratio);
	}
	else {
		printk("nootpotp \n");
	...
	printk("zgxadd2222 %s module_id=%d \n", __func__, module_id);
...
{
	//chengming.xiang modify Begin 2015-10-12 
	//correct R/G ,B/G Typical value according to sunny's golden report
	int RG_Ratio_Typical = 327, BG_Ratio_Typical = 308;
	//chengming.xiang modify end 2015-10-12 
	int rg, bg, R_gain, G_gain, B_gain, Base_gain, temp, i;
	// apply OTP WB Calibration
	if ((*otp_ptr).flag & 0x40) {

		//chengming.xiang modify Begin 2015-10-12 
		//To avoid division by zero exception ,if rg,bg equals zero ,set it to typical value
		 rg = (*otp_ptr).rg_ratio == 0 ?  RG_Ratio_Typical :(*otp_ptr).rg_ratio ;	 
		 bg = (*otp_ptr).bg_ratio == 0 ?  BG_Ratio_Typical :(*otp_ptr).bg_ratio ;		
		//chengming.xiang modify end 2015-10-12 
...
void OV8858GetModuleID(void)
{
	...
	read_otp(&otp_info);
	write_cmos_sensor(0x0100,0x00);
	printk("zgxadd111 %s module_id=%d\n", __func__, module_id);
}
