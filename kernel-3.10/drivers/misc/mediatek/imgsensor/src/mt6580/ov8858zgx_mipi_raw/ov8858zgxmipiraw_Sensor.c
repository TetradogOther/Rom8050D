...
#include "ov8858zgxmipiraw_Sensor.h"
...
void OV8858ZGXGetModuleID(void);
...
OV8858_VERSION ov8858zgxversion = OV8858R2A;
...
//  ;;
	...
	if (ov8858zgxversion == OV8858R1A){
		LOG_INF("inter r1asensor_init() ov8858zgxversion = %d(0=r2a,1=r1a)\n",ov8858zgxversion); 
		r1asensor_init();
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("ov8858zgx_ov8858zgx \n");
				OV8858ZGXGetModuleID();
				if((read_cmos_sensor(0x302A)) == 0XB2){
 				ov8858zgxversion = OV8858R2A;
				printk("zgx i2c write id: 0x%x, sensor id: 0x%x, ov8858zgxversion = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858zgxversion);	
			//	return ERROR_NONE;
				break;
				}
				else if((read_cmos_sensor(0x302A)) == 0XB1){
 				ov8858zgxversion = OV8858R1A;
				LOG_INF("zgx i2c write id: 0x%x, sensor id: 0x%x, ov8858zgxversion = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8858zgxversion);	
				//return ERROR_NONE;
				break;
				}
				else{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		*sensor_id = 0xFFFFFFFF;
		...
		{
			printk("ov8858zgx zgxadd %s module_id=%d \n", __func__, module_id);
			if (module_id != 0x0c) //0x01    //no fire otp
				{
				printk("ov8858zgx zgx1111 \n");
				*sensor_id = 0xFFFFFFFF;
				return ERROR_SENSOR_CONNECT_FAIL;
				}
	}
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	if((otp_flag & 0xc0) == 0x40) {
		printk("ov8858zgx group1 \n");
		addr = 0x7011; // base address of info group 1
	}
	else if((otp_flag & 0x30) == 0x10) {
		
		printk("ov8858zgx group2 \n");
	...
	printk("ov8858zgx zgxadd2222 %s module_id=%d \n", __func__, module_id);
...
void OV8858ZGXGetModuleID(void)
{
	...
	read_otp(&otp_info);
	write_cmos_sensor(0x0100,0x00);
	printk("ov8858zgx zgxadd111 %s module_id=%d\n", __func__, module_id);
}
...
UINT32 OV8858ZGX_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
...
UINT32 OV8858ZGXR2A_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
