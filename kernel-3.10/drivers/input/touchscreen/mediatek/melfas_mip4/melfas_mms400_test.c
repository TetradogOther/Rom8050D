...
* Pause IRQ
*/
static ssize_t mms_sys_irq_pause(struct device *dev, struct device_attribute *attr, char *buf) {
    struct mms_ts_info *info = dev_get_drvdata(dev);
    struct i2c_client *client = info->client;
    u8 data[255];
    int ret;
...
* Continue IRQ
*/
static ssize_t mms_sys_irq_continue(struct device *dev, struct device_attribute *attr, char *buf) {
    struct mms_ts_info *info = dev_get_drvdata(dev);
    struct i2c_client *client = info->client;
    u8 data[255];
    int ret;
...
static ssize_t mms_sys_gpio_irq(struct device *dev, struct device_attribute *attr, char *buf) {
    struct mms_ts_info *info = dev_get_drvdata(dev);

    if (!strcmp(attr->attr.name, "gpio_irq_input_pull_up")) {
        mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_EINT);
        mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_IN);
        mt_set_gpio_pull_enable(GPIO_CTP_EINT_PIN, GPIO_PULL_ENABLE);
        mt_set_gpio_pull_select(GPIO_CTP_EINT_PIN, GPIO_PULL_UP);
    } else if (!strcmp(attr->attr.name, "gpio_irq_input_pull_down")) {
        mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_EINT);
        mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_IN);
        mt_set_gpio_pull_enable(GPIO_CTP_EINT_PIN, GPIO_PULL_ENABLE);
        mt_set_gpio_pull_select(GPIO_CTP_EINT_PIN, GPIO_PULL_DOWN);
    } else if (!strcmp(attr->attr.name, "gpio_irq_input_no_pull")) {
        mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_EINT);
        mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_IN);
        mt_set_gpio_pull_enable(GPIO_CTP_EINT_PIN, GPIO_PULL_DISABLE);
    } else if (!strcmp(attr->attr.name, "gpio_irq_output_high")) {
        mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_GPIO);
        mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_CTP_EINT_PIN, GPIO_OUT_ONE);
    } else if (!strcmp(attr->attr.name, "gpio_irq_output_low")) {
        mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_GPIO);
        mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_OUT);
...
static DEVICE_ATTR(irq_pause, 0666, mms_sys_irq_pause, NULL);
static DEVICE_ATTR(irq_continue, 0666, mms_sys_irq_continue, NULL);
...
static DEVICE_ATTR(gpio_irq_input_pull_up, 0666, mms_sys_gpio_irq, NULL);
static DEVICE_ATTR(gpio_irq_input_pull_down, 0666, mms_sys_gpio_irq, NULL);
static DEVICE_ATTR(gpio_irq_input_no_pull, 0666, mms_sys_gpio_irq, NULL);
static DEVICE_ATTR(gpio_irq_output_high, 0666, mms_sys_gpio_irq, NULL);
static DEVICE_ATTR(gpio_irq_output_low, 0666, mms_sys_gpio_irq, NULL);
...
static struct attribute *mms_test_attr[] = {
    ...
    &dev_attr_irq_pause.attr,
    &dev_attr_irq_continue.attr,
    ...
    &dev_attr_gpio_irq_input_pull_up.attr,
    &dev_attr_gpio_irq_input_pull_down.attr,
    &dev_attr_gpio_irq_input_no_pull.attr,
    &dev_attr_gpio_irq_output_high.attr,
    &dev_attr_gpio_irq_output_low.attr,
    NULL,
};
